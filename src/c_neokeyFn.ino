...
void neokeySwirl() {
  if (!(depth == 0 & depthLast == 0)) {
    for (uint16_t i=0; i < numKeys; i++) {
      if (i < depth) {
        Neokey.pixels.setPixelColor(i, Wheel(((((numKeys-i) * 256 / numKeys) + neokeySwirlColor) & 255), neokeyBrightness));
      }
      else if (i < depthLast) {
    ...
    neokeySwirlColor++;       // increment swirl color to make colors cycle
  }
}
...
void neokeySpectrum() {
  if (!(depth == 0 & depthLast == 0)) {
    for (uint16_t i=0; i < numKeys; i++) {
      if (i < depth) {
        Neokey.pixels.setPixelColor(i, Wheel(neokeySwirlColor, neokeyBrightness));
      }
      else if (i < depthLast) {
        Neokey.pixels.setPixelColor(i, 0);
      }
...
void neokeySpectrumSingle() {
	if (!(depth == 0 & depthLast == 0)) {
		if (depth > 0) {
			Neokey.pixels.setPixelColor(depth-1, Wheel(neokeySwirlColor, neokeyBrightness));
		}
